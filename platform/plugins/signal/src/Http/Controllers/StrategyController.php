...
use Botble\Signal\Forms\StrategyForm;
...
use Botble\Signal\Tables\StrategyTable;
...
class StrategyController extends BaseController
{
     ...
     * @param StrategyInterface $strategyRepository
     */
    public function __construct(StrategyInterface $strategyRepository)
    {
     ...
     * @throws \Throwable
     */
    public function index(StrategyTable $table)
    {
        page_title()->setTitle(trans('plugins/signal::strategies.name'));
    ...
    {
        page_title()->setTitle(trans('plugins/signal::strategies.create'));

        return $formBuilder->create(StrategyForm::class)->renderForm();
    }
    ...
    {
        $strategy = $this->strategyRepository->createOrUpdate($request->input());

        event(new CreatedContentEvent(SIGNAL_MODULE_SCREEN_NAME, $request, $strategy));

        return $response
            ->setPreviousUrl(route('strategies.index'))
            ->setNextUrl(route('strategies.edit', $strategy->id))
